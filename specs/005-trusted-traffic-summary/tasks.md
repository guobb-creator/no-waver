...
- [ ] T004 新增高德核验 URL 构造。
  ...
  - 依赖：T003
  - 验证：单测覆盖中文地名、坐标和 mode。

- [ ] T005 增强地图路线摘要字段。
  - 文件：`lib/map-routing/types.ts`、`lib/map-routing/amap-client.ts`
  - 内容：尽量补充公共交通换乘次数、步行距离；保留现有 duration/distance。
  - 依赖：T003
...
- [ ] T010 更新地图异常处理。
  ...
  - 内容：占位符、地点不可识别、不在同城、距离过远、关键路线缺失时返回补充提示。
  - 依赖：T008
  - 验证：路由测试覆盖异常分支。
...
- [ ] T011 新增 `TrafficSummaryCard` 组件。
  ...
  - 内容：展示数据来源、候选卡片、路线项、交通判断、高德核验链接。
  - 依赖：T003
  - 验证：组件测试覆盖核心渲染。
...
| 高德核验跳转 | T004、T011、T017 | 每个候选目的地可打开高德路线 |
